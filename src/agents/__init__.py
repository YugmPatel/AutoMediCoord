"""
All AutoMediCoord Agents - Consolidated Implementation
"""
...
# Import from submodules (which have the actual code)
...
class BaseEDFlowAgent:
    """Base class for all AutoMediCoord agents"""
    
    def __init__(self, name: str, seed: str, port: Optional[int] = None):
        self.name = name
        
        # Create agent - both modes need port and endpoint
        endpoint = f"http://127.0.0.1:{port}/submit"
        
        if config.is_agentverse_mode():
            # Mailbox mode - needs port, endpoint, AND mailbox=True
            self.agent = Agent(
                name=name,
                seed=seed,
                port=port,
            ...
            logger.info(f"{name} agent initialized in MAILBOX mode (port {port})")
        else:
            # Local mode - port and endpoint only
            self.agent = Agent(
                name=name,
                seed=seed,
                port=port,
            ...
            logger.info(f"{name} agent initialized in LOCAL mode (port {port})")
