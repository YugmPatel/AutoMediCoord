...
class WebSocketManager:
    ...
    async def _simulate_agent_response(self, user_message: str):
        """Simulate agent response to user messages and handle patient arrivals"""
        await asyncio.sleep(2)  # Simulate processing delay
        
        try:
            # Check if message contains patient arrival information
            patient_case_created = await self._parse_and_create_patient_case(user_message)
            
            # If a patient case was created, simulate multi-agent coordination
            if patient_case_created:
                await self._simulate_multi_agent_coordination(patient_case_created, user_message)
            else:
                # Single agent response for non-patient messages
                await self._simulate_single_agent_response(user_message)
