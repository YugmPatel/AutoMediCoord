...
async def get_case_details(
    ...
    try:
        ...
        patient_data = ed_coordinator.active_patients.get(case_id)
        if not patient_data:
            raise HTTPException(status_code=404, detail=f"Case {case_id} not found")
...
async def discharge_case(
    ...
    try:
        ...
        discharged_patient = ed_coordinator.active_patients.pop(case_id)
...
async def get_case_statistics():
    ...
    try:
        ...
        cases = ed_coordinator.active_patients
...
async def get_case_timeline(
    ...
    try:
        ...
        # Generate timeline events (in real implementation, this would be tracked)
        arrival_time = patient_data.get('arrival_time', datetime.utcnow())
        if isinstance(arrival_time, str):
            arrival_time = datetime.fromisoformat(arrival_time.replace('Z', '+00:00'))
        ...
        if protocol == 'stemi':
            timeline.append({
                "timestamp": (arrival_time + timedelta(minutes=3)).isoformat(),
